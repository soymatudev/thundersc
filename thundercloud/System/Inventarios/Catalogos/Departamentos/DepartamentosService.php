...
class DepartamentosService
{
  ...
  public function crudSave($uu, $cc, $arrItems)
  {
    try {
      ...
      foreach($arrItems as $item) {
        $stmt = $this->conn->prepare("INSERT into ma_depar (descri) VALUES (:descri)");
      ...
      if (true) {
        ...
        if ($res !== false) {
            $this->thunderlog->writeLog("Execute => Correct => Departameto(s) generado con exito");
            ReturnEvent::returnResponse(0, "Departameto(s) generado con exito", "Departameto(s) generado con exito");
        } else {
  ...
  public function crudUpdate($uu, $cc, $cve_emple, $nombre_nuevo) {
    try {
      ...
      $stmt = $this->conn->prepare("UPDATE ma_depar SET descri = :descri WHERE clave = :clave");
  ...
  public function crudDelete($uu, $cc, $cve_emple) {
    try {
      ...
      $stmt = $this->conn->prepare("DELETE FROM ma_depar WHERE clave = :clave");
      $stmt->bindParam(':clave', $cve_emple, PDO::PARAM_STR);
      $result = $stmt->execute();
      $this->thunderlog->writeLog("Execute => " . $stmt->queryString);
...
function main()
{
  ...
  $componenteService = new DepartamentosService($data['uu']);
  $func = $data['function'];
