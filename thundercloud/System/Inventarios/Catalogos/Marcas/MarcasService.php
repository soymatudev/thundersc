...
ruta: thundersc/thundercloud/system/Inventarios/Catalogos/Marcas/MarcasService.php
===============================================================================*/
...
class MarcasService
{
  ...
  public function crudSave($uu, $cc, $arrItems)
  {
    try {
      ...
      foreach($arrItems as $item) {
        $stmt = $this->conn->prepare("INSERT into ma_marca (descri, type) VALUES (:descri, :type)");
      ...
      if (true) {
        ...
        if ($res !== false) {
            $this->thunderlog->writeLog("Execute => Correct => Marca(s) generado con exito");
            ReturnEvent::returnResponse(0, "Marca(s) generado con exito", "Marca(s) generado con exito");
        } else {
  ...
  public function crudUpdate($uu, $cc, $cve, $nombre_nuevo, $tipo) {
    try {
      ...
      $stmt = $this->conn->prepare("UPDATE ma_marca SET descri = :descri, type = :type WHERE clave = :clave");
      ...
      if (true) {
        $res = $this->conn->commit();
        if ($res !== false) {
            $this->thunderlog->writeLog("Execute => Correct => Marca actualizado con exito");
            ReturnEvent::returnResponse(0, "Marca actualizado con exito", "Marca actualizado con exito");
        } else {
          $this->thunderlog->writeLog("Execute => Incorrect");
          ReturnEvent::returnResponse(1, "Error en la consulta", "Error en la consulta");
        }
  ...
  public function crudDelete($uu, $cc, $cve) {
    try {
      ...
      $stmt = $this->conn->prepare("DELETE FROM ma_marca WHERE clave = :clave");
      $stmt->bindParam(':clave', $cve, PDO::PARAM_STR);
      $result = $stmt->execute();
      $this->thunderlog->writeLog("Execute => " . $stmt->queryString);
...
function main()
{
  ...
  $componenteService = new MarcasService($data['uu']);
  $func = $data['function'];
