...
class TemperatureEvent
{
    ...
    {
        list($this->sensor, $this->temp, $this->hum, $this->model, $this->version) = explode(",", $data);
    ...
    function BotTemp() {
        try {
            ...
            $temperatura = empty($this->temp) ? 0 : floatval($this->temp) - 1.5;
            $humedad = empty($this->hum) ? 0 : floatval($this->hum) - 1.5;
            $alias = $this->infosensor['alias'] ?: $this->sensor;

            $msg = $temperatura != null ? "🌡️ Sensor: $alias \n" . "Temperatura: $temperatura \n" . "Humedad: $humedad" : "Error: Sin Datos";
            $text = "\n $msg";


            $this->thunderlog->writeLog($msg);
            if ($temperatura > $this->infosensor['adc_1']) {
            ...
            } else if($temperatura < $this->infosensor['adc_3']) {
    ...
    function QueryEvent()
    {
        try {
            ...
            if (count($result) > 0) {
                ...
                $res->bindParam(':dato_1', $this->temp, PDO::PARAM_STR);
                $res->bindParam(':dato_2', $this->hum, PDO::PARAM_STR);
