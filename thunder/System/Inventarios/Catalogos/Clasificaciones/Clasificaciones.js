...
function init() {
    initPermi();
    Componentes.clasificaciones(uu, cc, "div-clasificacion");
    initCamp();
...
function initCamp() {
  ...
  setTimeout(() => { $("#select-clasificacion").prop("disabled", true); }, 250);
}
...
function crudAdd() {
  ...
  $("#select-clasificacion").prop("disabled", true);
  $("#grid").html("");
  arrItems = [];
  action = "INSERT";
}
...
function getEquipo() {
  ...
  let bridge = new Bridge(uu, cc, "System.Inventarios.Catalogos.Clasificaciones.ClasificacionesService.crudSearch", [args]);
  ...
  response
    ...
    .then((data) => {
      if(data.event > 0) {
        ...
        // Aquí se debe de hacer algo con la data
      }
    });
}
...
function crudSave() {
  if (action === "INSERT") {
      ...
      Swal.fire(
        Funciones.getConfigConfirmAlert("¿Realmente deseas guardar los cambios?", "question")
          ...
          if (result.value) {
            ...
            let bridge = new Bridge(uu, cc, "System.Inventarios.Catalogos.Clasificaciones.ClasificacionesService.crudSave", [arrItems]);
            let response = bridge.databriged();
    ...
    let clave = $("#select-clasificacion option:selected").val();
    let nombre = $("#descri").val().trim();
    let tipo = $("#select-tipo").val().trim();
    if (clave === "" || nombre === "") {
      Swal.fire({
        icon: "warning",
        title: "Campos vacios",
        text: "Debes seleccionar una clasificacion",
      });
    } else {
      Swal.fire(
        Funciones.getConfigConfirmAlert("¿Realmente deseas actualizar los cambios?", "question")
          ...
          if (result.value) {
            ...
            let bridge = new Bridge(uu, cc, "System.Inventarios.Catalogos.Clasificaciones.ClasificacionesService.crudUpdate", [clave, nombre, tipo]);
            let response = bridge.databriged();
...
function crudUpdate() {
  $("#btn_add, #btn_remove").prop("disabled", true);
  $("#select-clasificacion, #descri").prop("disabled", false);
...
function crudDelete() {
  ...
  if (clave === "") {
    ...
    Swal.fire(
      Funciones.getConfigConfirmAlert(`¿Realmente deseas eliminar la clasificacion?`, "question")
    ).then((result) => {
        if (result.value) {
          
          let bridge = new Bridge(uu, cc, "System.Inventarios.Catalogos.Clasificaciones.ClasificacionesService.crudDelete", [clave]);
          let response = bridge.databriged();
