 ...
 * @param {string} [filters.cve_alm] - Clave del almacén.
 * @param {string} [filters.f_ini] - Fecha de inicio del periodo.
 * @param {string} [filters.f_fin] - Fecha de fin del periodo.
 * @param {string} [filters.marca] - Clave de la marca.
 * @param {string} [filters.clasif] - Clave de la clasificación.
 * @param {string} [filters.empleado] - Clave del empleado.
 * @param {string} [filters.status] - Estatus del equipo ('1' para Activo).
 */
exports.getInventarioPorEquipo = async (filters) => {
    try {
        ...
        Logger.info('Executing query for inventory report with filters', { query, params });
        const data = await QueryHandler.execute(query, params);
 ...
 * Genera el reporte de inventario basado en filtros.
 * @param {object} filters - Objeto con los filtros para la consulta.
 * @param {string} [filters.serie] - serie del equipo.
 * @param {string} [filters.f_ini] - Fecha de inicio del periodo.
 * @param {string} [filters.f_fin] - Fecha de fin del periodo.
 * @param {string} [filters.marca] - Clave de la marca.
 * @param {string} [filters.clasif] - Clave de la clasificación.
 * @param {string} [filters.cod_inv] - Codigo de Inventario.
 * @param {string} [filters.status] - Estatus del equipo ('1' para Activo).
 */
exports.getIngresosPorEquipo = async (filters) => {
    try {
        ...
        Logger.info(`Executing query for inventory report with filters ${query}`, { query, params });
        const data = await QueryHandler.execute(query, params);
...
exports.generarReporteBackups = async () => {
    try {
        ...
        Logger.info('Executing query for DB backups report');
        ...
        Logger.error(`Error generating DB backups report: ${error.message}`);
        throw new Error('Failed to generate DB backups report.');
    }
};